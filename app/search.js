/* Concordance search — type a transliterated root or an English gloss, get
   ...
   Ported near-unchanged from Projects/Matthew/app/search.js -- fold() is
   already diacritic-generic (NFD + strip combining marks), so it works for
   ḥerem/naḥalah the same way it worked for aphiēmi. Only the storage key
   and hint text are Joshua-specific. */

import { getOccurrences, getThreadFor, resolveUnit } from "./threads.js?v=4";

const KEY = "joshua.search.q";

export function renderSearch(container, units) {
  const q0 = sessionStorage.getItem(KEY) || "";
  container.innerHTML = `
    <div class="search-view">
      <h1>Concordance</h1>
      <p class="search-hint">Type a transliterated root (<i>naḥalah</i>, or just <i>nahalah</i>)
        or an English gloss (<i>inheritance</i>). Searches every built unit.</p>
