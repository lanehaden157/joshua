...
## 2026-09-22 (part 2) — Unit 3 ported
- Ported `source-artifacts/joshua_03_translation.html` (Joshua 3:1–4:24, "Crossing the Jordan"). The artifact carried 8 open questions; got all 8 from Lane before porting and edited the source accordingly: ʾaron → "chest" (not the drafted "ark"), qadash/hitqaddeshu (3:5) → "set yourselves apart", ʾerets → "land" at 3:11/3:13/4:24, y'all-for-2pl extended to possessives ("y'all's"), berit locked to "covenant", the 4:22 Jacob/Gen 32:10 aside kept, unit 1's "sole" slug confirmed reused.
- Widened two tracked threads onto real occurrences: `rest` (5117 → +3240, the causative "lay down" at 4:3/4:8) and `strong` (2388 → +2389, the adjective at 4:24). Both in `data/roots.json` and `data/threads.json`, with new payoff entries for `possess` (3:10), `rest` (3:13), `cross` (3:17).
- Declined to promote the artifact's 3 well-evidenced book-wide candidates (`sole`, `lodge`, `fear`) to tracked threads this session — kept as unit-3 local roots. Promotion needs a hand-picked tracked-thread colour, which is Lane's call (`data/threads.json`'s `_note`), not something to invent mid-port; flagged for a future review rather than decided silently. `this-day` stays undecided too (228 word-hits book-wide — likely too frequent to carry signal, same shape as the declined `kol`).
- `translation-choices.md`: 4 new locked rows (ʾaron, berit, qadash/hitqaddeshu, ʾerets) plus the y'all's-possessives extension.
- Full build clean: 0 gap/wrong/stray/missing-data-w across 15 tracked threads over 3 built units; all `pipeline/test_*.py` pass. Smoke-tested unit 3 live in browser — legend, verse text, and notes all render the locked wording correctly.

## 2026-09-22 (part 3) — Tracked-thread colours made algorithmic; sole/lodge/fear promoted
- Lane corrected a wrong assumption: I'd left `sole`/`lodge`/`fear` as local roots because `data/threads.json`'s own note calls it "hand-authored," reading that as "colours are manually chosen." Lane's actual policy: colours are never hand-picked, full stop -- `assign_hues()` already does this for local roots, it just had no tracked-thread counterpart.
- New `pipeline/port_artifact.assign_tracked_colors(names)`: the missing counterpart to `assign_hues()` for promoting a thread. Checks a candidate colour against every existing tracked-thread colour plus every local root colour on record in `data/units.json`, globally (a tracked thread can appear in any future unit, so it can't be scoped to "just this unit" the way local roots are).
- Running it immediately surfaced a real capacity problem: the original 20-colour `WELL` palette was already almost exhausted (15 tracked threads + units 1+3's 18 combined local roots), and the fallback path had already produced one exact colour duplicate before this session (`servant`/`devote`, both `#8a2f3a`, pre-existing, not touched). Promoting 3 threads against it only produced near-duplicates (CIEDE2000 distance 5-7, below the DE_MIN=10 legibility floor).
- Lane's call: expand the palette instead of accepting degraded colours or promoting by hand. `WELL` grown from 20 to 65 -- the original 20 kept fixed and first (existing data references those exact hexes), 45 new colours generated by sampling CIE-Lab space in the same lightness/chroma band as the originals (not hand-picked pastels or neon), filtered for WCAG contrast >= 2.8 against `--bg`/`--panel` (the original well's own worst case), accepted greedily only when >= DE_MIN from everything already in the well.
