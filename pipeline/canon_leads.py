...
intertext pass.
    ...
    python pipeline/canon_leads.py 3            # -> canon-leads/canon-leads-unit-03.md
    python pipeline/canon_leads.py 3 --rare 30  # looser rare-word cutoff
    python pipeline/canon_leads.py --all        # every unit in data/units.json
    python pipeline/canon_leads.py              # built units + the next unbuilt one (build.py runs this)

LEADS, NOT CONCLUSIONS. This is the mechanical half of the intertext pass
(Lane, 2026-09-21): Claude Code finds where words recur, the Claude.ai
project decides which recurrences mean anything. It deliberately lists only
two kinds of hit, because listing every word is useless (measured on
Joshua 2: 147 lemmas, ~33,000 Torah verses once common words are in):

  * rare words -- a lemma in the passage that occurs in at most --rare verses
    across the whole Hebrew Bible (default 20), and somewhere outside Joshua
...
hand anyway), links by theme or type-scene, and anything outside the Hebrew
Bible, the New Testament included. Joshua 2's scarlet thread -> Tamar
(Gen 38:28) never shows up here: shani occurs dozens of times in the
tabernacle chapters. Those links are the project side's job.

Source: morphhb (pinned in package.json) -- the whole Hebrew Bible, read
from node_modules/morphhb/wlc. Lemma identity is the bare Strong's number
...
def load_bible(wlc=WLC):
    ...
    bible = {}
    for book in BOOKS:
        text = open(os.path.join(wlc, f"{book}.xml"), encoding="utf-8").read()
        verses = []
...
def load_glosses(path=LEXICON):
    """Strong's <def> phrases, joined -- a rough identifier for the reader,
    never a rendering (style reference §3: Strong's misleads as a gloss)."""
...
def passage_verses(bible, passage):
    ...
    return [(ref, words) for ref, words in bible["Josh"]
            if lo <= tuple(int(x) for x in ref.split(".")[1:]) <= hi]


def rare_leads(bible, freq, unit_verses, rare=RARE_DEFAULT):
    """Rare lemmas in the passage, with every occurrence outside Joshua."""
    ...
    for lid, info in seen.items():
        ...
        if hits:
            leads.append({"id": lid, "word": info["word"], "josh": info["refs"],
...
def phrase_leads(bible, freq, unit_verses):
    ...
    for ref, words in unit_verses:
        ...
        for run in runs:
            ...
            lead = leads.setdefault(key, {"words": span, "josh": [], "hits": hits})
            if ref not in lead["josh"]:
                lead["josh"].append(ref)
...
def render(n, passage, rare_list, phrase_list, glosses, rare):
    L = [f"# Canon leads — Unit {n} ({passage})", "",
         "Generated by `pipeline/canon_leads.py` from the Hebrew Bible (morphhb). "
    ...
    for p in phrase_list:
        ws = p["words"]
        josh = ", ".join(fmt_ref(r).replace("Josh ", "") for r in p["josh"])
        ids = " + ".join(f"H{w[0]}" for w in ws)
        gl = " / ".join(glosses.get(w[0], "?") for w in ws)
        L.append(f"- **{' '.join(translit(w) for w in ws)}** ({ids}: {gl}) — Josh {josh}")
    ...
    for r in rare_list:
        josh = ", ".join(fmt_ref(x).replace("Josh ", "") for x in r["josh"])
        ...
        L.append(f"- **{translit(r['word'])}** (H{r['id']}, {glosses.get(r['id'], '?')}) — "
                 f"Josh {josh}; {r['freq']} verses in the Hebrew Bible")
        if torah:
...
def build(n, rare=RARE_DEFAULT, bible=None, freq=None, glosses=None, out_dir=OUT_DIR):
    units = json.load(open(UNITS_JSON, encoding="utf-8"))["units"]
    row = next((u for u in units if u["n"] == n), None)
    if row is None:
