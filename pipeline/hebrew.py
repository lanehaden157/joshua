...
Scheme (a summary -- pipeline/test_hebrew.py's expected values are the
...
_WORDS_TSV = os.path.join(HERE, "..", "Joshua-words.tsv")
...
_ref_rows_cache = None


def _load_rows_by_ref():
    global _ref_rows_cache
    if _ref_rows_cache is None:
        rows = {}
        with open(_WORDS_TSV, encoding="utf-8") as f:
            reader = csv.DictReader(f, delimiter="\t")
            for row in reader:
                rows.setdefault(row["ref"], []).append(row)
        _ref_rows_cache = rows
    return _ref_rows_cache


def _to_tsv_ref(ref: str) -> str:
    """'Josh 1:1' -> 'Josh.1.1' (Joshua-words.tsv's ref spelling)."""
    book, rest = ref.split(" ", 1)
    chap, verse = rest.split(":")
    return f"{book}.{chap}.{verse}"


def transliterate_ref(ref: str) -> str:
    """Transliterate every word of a verse, in Joshua-words.tsv order,
    space-joined, applying lemma-keyed overrides. `ref` is the spoken form
    ("Josh 1:1"), matching Joshua-reading.txt. Word-based, not raw-text-
    ...
    rows = _load_rows_by_ref().get(tsv_ref)
    if not rows:
        raise ValueError(f"no Joshua-words.tsv rows for ref {ref!r} ({tsv_ref!r})")
    return " ".join(
...
if __name__ == "__main__":
    # Quick manual sanity check. The authoritative regression test, with
    # real word ids pulled from Joshua-words.tsv, is pipeline/test_hebrew.py.
    samples = [
        "יְהוָה",  # YHWH, bare -- deterministic (no override without a lemma)
        "כָּל",  # kol, bare -- deterministic
        "וְ/כָל",  # "and all": prefix morpheme + root, "-"-joined
        "אֶל־יְהוֹשֻׁעַ",  # phrase w/ maqqef
    ]
    for t in samples:
        print(f"{t}  ->  {transliterate(t)}")
    print(transliterate_ref("Josh 1:1"))
