...
Hebrew fork of Projects/Matthew/pipeline/apply_retrofit.py -- this module is
genuinely language-neutral (Port analysis.md §1.3: pure HTML-text
manipulation, no Greek/Hebrew-specific code), so the ops themselves port
unchanged. The one real addition: Joshua's `data-root` spans on TRACKED
threads must also carry `data-w` (the OSHB word id, style reference §2) --
Matthew's spans never needed one. `apply_add` and `apply_retag` accept an
optional `"w"` field in their retrofit entry and inject/update `data-w`
alongside `data-root`. `retag_word` (a whole-unit regex sweep over MULTIPLE
spans at once) deliberately does NOT accept `w` -- a bulk reclassification
changes which root a span belongs to, not which word it points at, so any
`data-w` a matched span already carries is left untouched. If a `retag_word`
sweep would newly classify spans under a tracked thread that don't already
carry `data-w`, that's a real gap this module doesn't try to paper over --
use per-verse `retag` (which does support `w`) for that case instead.

Idempotent -- every op no-ops if already applied, so build.py re-runs it each
time. The committed fragments already have all of this; the file is the
record of what was done, and the safety net if port_artifact.py re-derives a
fragment.
