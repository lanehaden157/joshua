"""Audit tracked-thread tag coverage against the Hebrew, by set arithmetic
over word ids -- not by matching Hebrew strings against each other.

Rewritten for phase-0.6-plan.md §D: root identity is now a hand-curated
set of Strong's/lemma ids in data/roots.json (joshua_study_style_reference.md
§2), and every tracked-thread span in a fragment carries the OSHB word id
it refers to (`data-w`). The audit's whole job is now: what does the
source have (every non-Ketiv word whose lemma is in a root's id set,
within a unit's passage) minus what the fragment tagged (every `data-w`
under that root's spans) -- and the reverse, what's tagged that shouldn't
be. No Hebrew string is ever compared to another Hebrew string here.

This replaces the substring-stem matching this module used to do
(`_compile_stems`, `hebrew_hits`, `strip_prefixes`, `HEBREW_PREFIX_LETTERS`)
-- measured against real OSHB Joshua, stem recall on weak roots was bad
enough to be disqualifying (*natan* 42%, *qum* 24%, *nakah* 3%, Yehoshuaʿ
0% -- style reference §2). `pipeline/thread-stems.json` is gone; roots
live in `data/roots.json` now (see `pipeline/roots.py`).

Kept from the stem-matching version, unchanged in spirit:
- Source file and line-prefix are read from data/units.json's `book`
  field, self-checked against the file's own first line (see
  `_text_file_and_prefix`).
- The CLI shape (`--unit`, `--stub`) and `coverage_for_unit`/
  `coverage_for_fragment` for the Phase 3 porter.
- The per-verse tagged-root view (`tagged_map`), now used only for LOCAL
  roots -- a unit's own `roots[]` entries that aren't tracked threads
  don't carry `data-w` (joshua_study_style_reference.md §2: "local roots
  ... don't need data-w; the audit counts those per verse") and have no
  id set to check coverage against, so they're reported informationally,
  not gap/wrong/stray audited.

Ketiv/Qere (§A4): the artifact tags the Qere id, and this audit counts
the Qere id. Joshua-words.tsv keeps both rows (same ref), with the Ketiv
row written unpointed (per build_reading.py) -- `load_words()` detects and
drops Ketiv rows by that adjacency pattern (same ref, unpointed row
immediately followed by a pointed one), verified against the full corpus
to find exactly the documented 32 pairs and zero exceptions.

    python pipeline/audit_thread_coverage.py             # every tracked thread
    python pipeline/audit_thread_coverage.py give devote  # just these
    python pipeline/audit_thread_coverage.py --stub give  # retrofit-tags stubs
                                                           # for give's gaps
    python pipeline/audit_thread_coverage.py --ids kol give
    ...
    python pipeline/audit_thread_coverage.py --unit unit-06
...
WORDS_TSV = os.path.join(ROOT, "Joshua-words.tsv")
...
def _text_file_and_prefix():
    """Read data/units.json's `book` field and derive the source filename
    and line-ref prefix from it. Self-checks the derived prefix against
    the file's own first line rather than trusting the derivation
    blindly."""
    units_path = os.path.join(DATA, "units.json")
    if not os.path.exists(units_path):
        sys.exit(f"[audit] {units_path} not found -- need its `book` field "
                  f"to know which source text and line-prefix to read.")
    book = json.load(open(units_path, encoding="utf-8")).get("book")
    if not book:
        sys.exit(f"[audit] {units_path} has no `book` field.")

    path = os.path.join(ROOT, f"{book}-reading.txt")
    prefix = book[:4]
    if not os.path.exists(path):
        sys.exit(f"[audit] {path} not found (derived from units.json's "
                  f"book='{book}' as '<book>-reading.txt').")
    first_line = open(path, encoding="utf-8").readline()
    if not first_line.startswith(prefix + " "):
        sys.exit(f"[audit] derived prefix '{prefix}' (book[:4]) doesn't match "
                  f"{path}'s own first line ({first_line[:20]!r}...). "
                  f"book[:4] isn't this book's real abbreviation -- fix the "
                  f"derivation in _text_file_and_prefix(), don't hardcode "
                  f"around it.")
    return path, prefix


def load_hebrew():
    """[(ch, v, line_text), ...] straight from the reading text -- still
    needed for the local-root per-verse view (tagged_map/expected_seq) and
    for verse_text()."""
    path, prefix = _text_file_and_prefix()
    line_re = re.compile(re.escape(prefix) + r" (\d+):(\d+)\t(.*)")
    verses = []
    ...
    if not verses:
        sys.exit(f"[audit] {path} parsed to zero verses -- wrong format?")
    return verses
...
def load_words():
    """Every Joshua-words.tsv row, Ketiv rows dropped (§A4), as dicts with
    an added integer (ch, v) parsed from `ref`. Order preserved -- needed
    to detect Ketiv/Qere adjacency, which only holds in file order."""
    raw = []
    ...
    for i in range(len(raw) - 1):
        a, b = raw[i], raw[i + 1]
        if (a["ref"] == b["ref"] and not _has_niqqud(a["surface"])
                and _has_niqqud(b["surface"])):
            is_ketiv[i] = True
...
def coverage_for_fragment(slug, html, passage, threads_json=None, roots_json=None):
    ...
    "warnings": [...]}."""
    threads_json = threads_json or json.load(
        open(os.path.join(DATA, "threads.json"), encoding="utf-8"))
        ...
        for wid in sorted(tagged_ids):
            ...
            if row is None:
                strays.append({"thread": tid, "root": root_slug, "word_id": wid,
                                "reason": "does not exist in Joshua-words.tsv"})
                continue
            cv = (row["ch"], row["v"])
...
def audit(only=None, stub_for=None, unit_slugs=None):
    threads_json = json.load(open(os.path.join(DATA, "threads.json"), encoding="utf-8"))
...
def main():
    ...
    if ids_mode:
        ...
        sys.exit(ids_report(ids_targets or (only or [])))

    scope = f" — {', '.join(unit_slugs)}" if unit_slugs else " vs. built fragments"
    print(f"thread coverage audit — Hebrew{scope}\n")
